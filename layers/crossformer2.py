...
class Attention(nn.Module):
    ...
    def transpose_for_scores(self, x):
        ...
        new_x_shape = x.size()[:-1] + (24, 32)
        x = x.view(*new_x_shape)
        return x.permute(0, 2, 1, 3)

    def forward(self, x, mask=None,flag=False):
        """
        Args:
...
class CrossFormerBlock(nn.Module):
    ...
    def forward(self, x,flag=False):
        ...
        x,wg = self.attn(x, mask=self.attn_mask,flag=flag)  # nW*B, G*G, C
    ...
    def forward(self, x,flag=False):
        for blk in self.blocks:
            if self.use_checkpoint:
                x,wg = checkpoint.checkpoint(blk, x)
            else:
                x,wg = blk(x,flag)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=[4, 8, 16, 32], in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
...
class CrossFormer(nn.Module):
    ...
    def __init__(self, hash_bit=64,label=13,img_size=224, patch_size=[4, 8, 16, 32], in_chans=3, num_classes=1000,
        ...
        self.part_select = Location_RELA()#Part_Attention
        
        self.avgpool = nn.AdaptiveAvgPool1d(1)
       
        self.norm1 = norm_layer(384)#384
        
        
        self.apply(self._init_weights)
        
        self.ol=nn.Linear(384, 256)
    ...
    def forward_features(self, x):
        ...
        x = self.part_select(x)
        x = self.norm1(x)  # B L C
        x = self.avgpool(x.transpose(1, 2))  # B C 1
        x = torch.flatten(x, 1)
        
        #xs = self.norm1(xs)  # B L C
        #xs = self.avgpool(xs.transpose(1, 2))  # B C 1
